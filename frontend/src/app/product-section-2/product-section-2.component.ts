...
export class ProductSection2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._service.getProducts(1, 100).subscribe({
      next: (data) => {
        ...
        // Filter products for tre mây
        const treMayProducts = this.products.filter(product => product.type === 'tre_may');
        this.displayedProducts = treMayProducts.slice(0, this.initialDisplayCount);
  ...
  showMore(): void {
    ...
    const treMayProducts = this.products.filter(product => product.type === 'tre_may');
    const additionalProducts = treMayProducts.slice(currentLength, currentLength + this.loadMoreCount);
